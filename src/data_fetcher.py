...
class DataFetcher:
    ...
    def plot(self) -> None:
        plot_file_source_name = self.source_name.lower().replace(" ", "_").replace(",", "")
        plt = self.river_df.plot(x="ds", y="y", figsize=(12, 6), grid=True)
        plt.set_xlabel("Date")
        plt.set_ylabel("Flow (cfs)")
        plt.set_title(f"{self.source_name}")
        plt.figure.savefig(os.path.join("plots", "historical_flow", f"{plot_file_source_name}_historical.png"))
        self.session.close()
